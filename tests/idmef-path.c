...
static void set_value_check(idmef_message_t *idmef, const char *paths,
                            const char *str_value, requiem_bool_t verify_get)
{
        idmef_path_t *path;
        idmef_value_t *value;
        requiem_string_t *str;
        requiem_string_t *res;

        assert(idmef_path_new_fast(&path, paths) == 0);
        assert(requiem_string_new_ref(&str, str_value) == 0);
        ...
        assert(requiem_string_new(&res) == 0);
        assert(idmef_value_to_string(value, res) >= 0);
        assert(strcmp(str_value, requiem_string_get_string(res)) == 0);
        requiem_string_destroy(res);
...
int main(void)
{
        ...
        struct {
                const char *path;
                int depth;
                requiem_bool_t has_list;
                requiem_bool_t ambiguous;
                requiem_bool_t successful;
        } plist[] = {
