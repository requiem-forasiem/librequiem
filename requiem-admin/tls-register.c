...
static int remove_old_certificate(const char *filename, uint64_t dn, uint64_t issuer_dn)
{
        ...
        ret = _requiem_load_file(filename, &data, &size);
        if ( ret < 0 ) {
                if ( requiem_error_get_code(ret) == REQUIEM_ERROR_ENOENT )
                        return 0;
        ...
        if ( ret < 0 ) {
                _requiem_unload_file(data, size);
                return ret;
        }
        ...
        while ( 1 ) {
                ...
                requiem_string_sprintf(out, "-----BEGIN CERTIFICATE-----\n%s-----END CERTIFICATE-----\n", buf);

                datum.size = requiem_string_get_len(out);
                datum.data = (unsigned char *) const_to_char(requiem_string_get_string(out));
                data += datum.size;
                ...
                if ( ret == 0 )
                        requiem_log_debug(1, "Removing old certificate with subject=%"
                                REQUIEM_PRIu64 " issuer=%" REQUIEM_PRIu64 ".\n", dn, issuer_dn);
                else {
        ...
        fclose(fd);
        _requiem_unload_file(datap, size);
        requiem_string_destroy(out);
...
static int save_buf(const char *filename, requiem_uid_t uid, requiem_gid_t gid, const unsigned char *buf, size_t size)
{
        ssize_t sret;
        int fd, ret, flags = 0;
...
static int crt_import(gnutls_x509_crt *crt, const char *filename)
{
        ...
        ret = _requiem_load_file(filename, &data.data, &dsize);
        if ( ret < 0 ) {
                fprintf(stderr, "error loading '%s': %s.\n", filename, requiem_strerror(ret));
                return ret;
        }
        ...
        _requiem_unload_file(data.data, dsize);
        return ret;
}
...
int tls_load_ca_certificate(requiem_client_profile_t *cp, gnutls_x509_privkey key, gnutls_x509_crt *crt)
{
        int ret;
        char filename[256];

        requiem_client_profile_get_tls_server_ca_cert_filename(cp, filename, sizeof(filename));
                                   ...
                                   gnutls_x509_crt *crt)
{
        ...
        requiem_client_profile_get_tls_server_keycert_filename(cp, filename, sizeof(filename));
        ...
        *crt = generate_signed_certificate(cp, requiem_client_profile_get_analyzerid(cp), cacrt, cakey, crq);
        if ( ! *crt )
                return -1;
