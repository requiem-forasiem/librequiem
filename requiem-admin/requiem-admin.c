...
static REQUIEM_LIST(rm_dir_list);
static requiem_option_t *parentopt;
static requiem_client_profile_t *profile;
static char *addr = NULL, *one_shot_passwd = NULL, *arg_command = NULL;
static requiem_bool_t gid_set = FALSE, uid_set = FALSE, detailed_listing = FALSE;
static requiem_bool_t prompt_passwd = FALSE, server_keepalive = FALSE, pass_from_stdin = FALSE;


int generated_key_size = 0;
requiem_bool_t server_confirm = TRUE;
...
static const char *get_tls_config(void)
{
        ...
        requiem_client_profile_get_default_config_dirname(NULL, buf, sizeof(buf));
...
static int chown_cb(const char *filename, const struct stat *st, int flag)
{
        ...
        uid = uid_set ? requiem_client_profile_get_uid(profile) : (requiem_uid_t) -1;
        gid = gid_set ? requiem_client_profile_get_gid(profile) : (requiem_gid_t) -1;
...
static int do_chown(const char *name)
{
        ...
        requiem_client_profile_get_profile_dirname(profile, dirname, sizeof(dirname));
        ret = ftw(dirname, chown_cb, 10);
        if ( ret < 0 )
                return -1;

        requiem_client_profile_get_backup_dirname(profile, dirname, sizeof(dirname));
        ret = ftw(dirname, chown_cb, 10);
        if ( ret < 0 )
                return -1;

        fprintf(stderr, "Changed '%s' ownership to UID:%d GID:%d.\n", name,
                uid_set ? (int) requiem_client_profile_get_uid(profile) : -1,
                gid_set ? (int) requiem_client_profile_get_gid(profile) : -1);
...
"* [Please press enter if this is what you intend to do]\n",
                (int) requiem_client_profile_get_uid(profile), (int) requiem_client_profile_get_gid(profile));
...
"* [Please press enter if this is what you intend to do]\n",
                requiem_client_profile_get_name(profile), exist_uid, exist_gid,
                requiem_client_profile_get_name(profile),
                (int) requiem_client_profile_get_uid(profile), (int) requiem_client_profile_get_gid(profile));

        while ( getchar() != '\n' );

        return do_chown(requiem_client_profile_get_name(profile));
}
...
"Example: rename requiem-lml-old requiem-lml-new\n\n"
...
{
        fprintf(stderr,
"Usage  : registration-server <Requiem-Manager profile> [options]\n"
"Example: registration-server requiem-manager\n\n"

"Launch a registration server for the specified Requiem-Manager profile. The\n"
"profile will be created if it does not exist. Registered analyzers will be able\n"
"to communicate with Requiem-Manager instance using this profile.\n\n"
...
"Example: register requiem-lml \"idmef:w\" 192.168.0.1\n\n"
...
"Example: chown requiem-lml --uid lmluser --gid lmlgroup\n\n"
...
"Example: revoke requiem-manager 227879253605921\n\n"
...
"Print the messages within a Requiem IDMEF binary file (example: failover file)\n"
...
{
        fprintf(stderr,
"Usage  : send <analyzer profile> <Requiem-Manager address> <file>\n"
"Example: send requiem-lml 192.168.0.1 /path/to/file1 /path/to/file2 /path/to/fileN\n\n"

"Send the messages within a Requiem IDMEF binary file (example: failover file)\n"
"to the specified Requiem-Manager address. The specified profile is used for\n"
"authentication.\n\n"
...
#if !((defined _WIN32 || defined __WIN32__) && !defined __CYGWIN__)
static int set_uid(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
        requiem_uid_t uid;
        const char *p;
        struct passwd *pw;
        ...
        uid_set = TRUE;
        requiem_client_profile_set_uid(profile, uid);
...
static int set_gid(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
        requiem_gid_t gid;
        const char *p;
        struct group *grp;
        ...
        gid_set = TRUE;
        requiem_client_profile_set_gid(profile, gid);
...
static int set_server_keepalive(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
...
static int set_passwd(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
...
static int set_passwd_file(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
        FILE *fd;
        size_t len;
        char buf[1024], *ptr;
...
static int set_prompt_passwd(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
        prompt_passwd = TRUE;
        return 0;
}
...
static int set_server_no_confirm(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
        server_confirm = FALSE;
        return 0;
}
...
static int set_server_listen_address(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
        int ret;

        ret = requiem_parse_address(optarg, &addr, &port);
        if ( ret < 0 )
...
static int set_key_len(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
...
static int set_cert_lifetime(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
...
static int set_ca_cert_lifetime(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
...
static int set_long_listing(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
...
{
#if !((defined _WIN32 || defined __WIN32__) && !defined __CYGWIN__)
        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 'u', "uid",
                           NULL, REQUIEM_OPTION_ARGUMENT_REQUIRED, set_uid, NULL);

        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 'g', "gid",
                           NULL, REQUIEM_OPTION_ARGUMENT_REQUIRED, set_gid, NULL);
#endif
}
...
{
        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 'o', "offset",
                           NULL, REQUIEM_OPTION_ARGUMENT_REQUIRED, set_offset, NULL);

        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 'c', "count",
...
{
        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 'l', "long",
                           NULL, REQUIEM_OPTION_ARGUMENT_NONE, set_long_listing, NULL);
}
...
static int read_tls_setting(void)
{
        ...
        if ( ret < 0 ) {
                requiem_perror(ret, "could not open %s", get_tls_config());
                return -1;
        }
...
static void setup_tls_options(void)
{
        ...
        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 0, "key-len",
                           NULL, REQUIEM_OPTION_ARGUMENT_REQUIRED, set_key_len, NULL);

        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 0, "cert-lifetime",
                           NULL, REQUIEM_OPTION_ARGUMENT_REQUIRED, set_cert_lifetime, NULL);

        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 0, "ca-cert-lifetime",
                           NULL, REQUIEM_OPTION_ARGUMENT_REQUIRED, set_ca_cert_lifetime, NULL);
}
 ...
 * analyzer.
 */
static int create_template_config_file(requiem_client_profile_t *profile)
{
        int ret;
        FILE *fd, *tfd;
        size_t len, wlen;
        ...
        requiem_client_profile_get_default_config_dirname(profile, dirname, sizeof(dirname));
        snprintf(dirname + strlen(dirname), sizeof(dirname) - strlen(dirname), "/idmef-client.conf");

        requiem_client_profile_get_config_filename(profile, filename, sizeof(filename));
        ...
        ret = fchown(fileno(fd), requiem_client_profile_get_uid(profile), requiem_client_profile_get_gid(profile));
        if ( ret < 0 )
...
static int register_sensor_ident(const char *name, uint64_t *ident)
{
        ...
        *ident = generate_analyzerid();
        ...
        requiem_client_profile_get_analyzerid_filename(profile, filename, sizeof(filename));
        ...
        if ( already_exist == 0 ) {
                ...
                if ( ! sscanf(buf, "%" REQUIEM_SCNu64, ident) ) {
                        fclose(fd);
                        unlink(filename);
                        return register_sensor_ident(name, ident);
                }
...
static int anon_check_passwd(requiem_io_t *fd, char *passwd)
{
        int ret;
        ssize_t size;
        unsigned char *rbuf;

        size = requiem_io_write_delimited(fd, passwd, strlen(passwd) + 1);
        if ( size < 0 ) {
                fprintf(stderr, "error sending authentication token: %s.\n", requiem_strerror(size));
                return -1;
        }

        size = requiem_io_read_delimited(fd, &rbuf);
        if ( size < 2 ) {
...
static gnutls_session new_tls_session(int sock, char *passwd)
{
...
#ifdef GNUTLS_SRP_ENABLED
        {
                ...
                gnutls_srp_set_client_credentials(srpcred, "requiem-adduser", passwd);
...
static requiem_io_t *connect_manager(const char *addr, unsigned int port, char *passwd)
{
        int ret, sock;
        requiem_io_t *fd;
        gnutls_session session;
        ...
        requiem_io_set_tls_io(fd, session);
...
static int create_directory(requiem_client_profile_t *profile, const char *dirname, int flags)
{
        int ret;
        ...
        ret = chown(dirname, requiem_client_profile_get_uid(profile), requiem_client_profile_get_gid(profile));
        if ( ret < 0 ) {
                fprintf(stderr, "could not chown %s to %d:%d: %s.\n", dirname,
                        (int) requiem_client_profile_get_uid(profile),
                        (int) requiem_client_profile_get_gid(profile), strerror(errno));
                return -1;
        }
...
static int setup_analyzer_files(requiem_client_profile_t *profile, uint64_t *analyzerid,
                                gnutls_x509_privkey *key, gnutls_x509_crt *crt)
{
        ...
        requiem_client_profile_get_profile_dirname(profile, buf, sizeof(buf));
        ...
        *key = tls_load_privkey(profile);
        ...
        name = requiem_client_profile_get_name(profile);
        ...
        requiem_client_profile_set_analyzerid(profile, *analyzerid);

        requiem_client_profile_get_backup_dirname(profile, buf, sizeof(buf));
...
static int add_to_rm_dir_list(const char *filename)
{
        ...
        requiem_list_add(&rm_dir_list, &new->list);
...
static int flush_rm_dir_list(void)
{
        ...
        requiem_list_for_each_safe(&rm_dir_list, tmp, bkp) {
                dir = requiem_list_entry(tmp, struct rm_dir_s, list);
...
static int delete_profile(requiem_client_profile_t *profile)
{
        int ret;
        char buf[PATH_MAX];
...
static int rename_cmd(int argc, char **argv)
{
        ...
        requiem_client_profile_t *sprofile, *dprofile;

        i = ret = requiem_option_read(parentopt, NULL, &argc, argv, &err, NULL);
        if ( ret < 0 ) {
                requiem_perror(ret, "Option error");
                return -1;
        }
        ...
        ret = requiem_client_profile_new(&sprofile, sname);
        if ( ret < 0 ) {
                fprintf(stderr, "Error opening profile '%s': %s\n", sname, requiem_strerror(ret));
                return -1;
        }

        ret = requiem_client_profile_new(&sprofile, dname);
        if ( ret == 0 ) {
                fprintf(stderr, "Could not rename profile '%s' to '%s': profile '%s' already exist.\n", sname, dname, dname);
                return -1;
        }

        ret = _requiem_client_profile_new(&dprofile);
        requiem_client_profile_set_name(dprofile, dname);

        requiem_client_profile_get_profile_dirname(sprofile, spath, sizeof(spath));
        requiem_client_profile_get_profile_dirname(dprofile, dpath, sizeof(dpath));
        ...
        requiem_client_profile_get_backup_dirname(sprofile, spath, sizeof(spath));
        requiem_client_profile_get_backup_dirname(dprofile, dpath, sizeof(dpath));
...
static int get_existing_profile_owner(const char *buf)
{
        ...
        if ( ! uid_set )
                requiem_client_profile_set_uid(profile, st.st_uid);

        if ( ! gid_set )
                requiem_client_profile_set_gid(profile, st.st_gid);

        if ( (uid_set && st.st_uid != requiem_client_profile_get_uid(profile)) ||
             (gid_set && st.st_gid != requiem_client_profile_get_gid(profile)) )
                        ...
                        gnutls_x509_crt *crt, gnutls_x509_crt *ca_crt)
{
        ...
        requiem_client_profile_set_name(profile, name);
        requiem_client_profile_get_profile_dirname(profile, buf, sizeof(buf));
        ...
        if ( ret < 0 ) {
                ...
                if ( ! uid_set || ! gid_set ) {
                        if ( ! uid_set )
                                requiem_client_profile_set_uid(profile, getuid());

                        if ( ! gid_set )
                                requiem_client_profile_set_gid(profile, getgid());
                }
        }
...
static int add_cmd(int argc, char **argv)
{
        ...
        requiem_client_profile_t *testprofile;

        ret = _requiem_client_profile_new(&profile);
        ...
        ret = requiem_client_profile_new(&testprofile, argv[i]);
        if ( ret == 0 ) {
        ...
        fprintf(stderr, "Created profile '%s' with analyzerID '%" REQUIEM_PRIu64 "'.\n", argv[i], analyzerid);
...
static int chown_cmd(int argc, char **argv)
{
        ...
        _requiem_client_profile_new(&profile);
        ...
        ret = requiem_client_profile_set_name(profile, argv[i]);
        if ( ret < 0 ) {
                fprintf(stderr, "Error loading analyzer profile '%s': %s.\n\n", argv[i], requiem_strerror(ret));
                return -1;
        }
...
static int register_cmd(int argc, char **argv)
{
        ...
        requiem_connection_permission_t permission_bits;

        ret = _requiem_client_profile_new(&profile);
        setup_permission_options();
        setup_tls_options();

        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 0, "passwd", NULL,
                           REQUIEM_OPTION_ARGUMENT_REQUIRED, set_passwd, NULL);

        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 0, "passwd-file", NULL,
                           REQUIEM_OPTION_ARGUMENT_REQUIRED, set_passwd_file, NULL);
        ...
        ret = requiem_connection_permission_new_from_string(&permission_bits, argv[i + 1]);
        if ( ret < 0 ) {
                fprintf(stderr, "could not parse permission: %s.\n", requiem_strerror(ret));
                return -1;
        }

        ret = requiem_parse_address(strdup(argv[i + 2]), &addr, &port);
        if ( ret < 0 ) {
        ...
        fprintf(stderr,
                ...
                "example: \"%s registration-server requiem-manager\"\n\n",
...
static int registration_server_cmd(int argc, char **argv)
{
        ...
        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 'k', "keepalive", NULL,
                           REQUIEM_OPTION_ARGUMENT_NONE, set_server_keepalive, NULL);
        ...
        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 'p', "prompt", NULL,
                           REQUIEM_OPTION_ARGUMENT_NONE, set_prompt_passwd, NULL);

        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 'n', "no-confirm", NULL,
                           REQUIEM_OPTION_ARGUMENT_NONE, set_server_no_confirm, NULL);

        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 'l', "listen", NULL,
                           REQUIEM_OPTION_ARGUMENT_REQUIRED, set_server_listen_address, NULL);
...
static int revoke_cmd(int argc, char **argv)
{
        ...
        ret = requiem_client_profile_new(&profile, argv[i]);
        if ( ret < 0 ) {
                fprintf(stderr, "Error opening profile '%s': %s.\n", argv[i], requiem_strerror(ret));
                return -1;
        }
...
static int read_messages(const char *filename, requiem_io_t *io,
...
static int print_cmd(int argc, char **argv)
{
        ...
        setup_read_options();
        i = ret = requiem_option_read(parentopt, NULL, &argc, argv, &str, NULL);
        ...
        ret = requiem_io_new(&out);
        if ( ret < 0 )
                return ret;

        requiem_io_set_file_io(out, stdout);
...
{
        requiem_client_send_idmef(data, idmef);
        return 0;
}
...
static int send_cmd(int argc, char **argv)
{
        ...
        ret = requiem_client_new(&client, argv[i]);
        if ( ret < 0 )
                return ret;

        flags = requiem_client_get_flags(client);
        flags &= ~REQUIEM_CLIENT_FLAGS_HEARTBEAT;
        requiem_client_set_flags(client, flags);

        pool = requiem_client_get_connection_pool(client);
        flags = requiem_connection_pool_get_flags(pool);

        ret = requiem_client_init(client);

        flags &= ~(REQUIEM_CONNECTION_POOL_FLAGS_RECONNECT|REQUIEM_CONNECTION_POOL_FLAGS_FAILOVER);
        requiem_connection_pool_set_flags(pool, flags);
        requiem_connection_pool_set_connection_string(pool, argv[i + 1]);

        ret = requiem_client_start(client);
        if ( ret < 0 ) {
                fprintf(stderr, "Error starting requiem-client: %s.\n", requiem_strerror(ret));
                return ret;
        }
        ...
        requiem_client_destroy(client, REQUIEM_CLIENT_EXIT_STATUS_SUCCESS);
        return ret;
}


static REQUIEM_LIST(print_list);
                      ...
                      const char *analyzerid, const char *perm, const char *tanalyzerid)
{
        ...
        requiem_list_add_tail(&print_list, &ent->list);
}
...
static void print_added(void)
{
        ...
        requiem_list_for_each(&print_list, tmp) {
                ent = requiem_list_entry(tmp, print_entry_t, list);
        ...
        requiem_list_for_each_safe(&print_list, tmp, bkp) {
                ent = requiem_list_entry(tmp, print_entry_t, list);
...
static int list_cmd(int argc, char **argv)
{
        ...
        requiem_client_profile_get_profile_dirname(NULL, dirname, sizeof(dirname));
        ...
        while ( (dh = readdir(dir)) ) {
                ...
                ret = _requiem_load_file(buf, &data.data, &size);
                if ( ret < 0 ) {
                ...
                ret = _requiem_tls_crt_list_import(certs, &cert_max, &data, GNUTLS_X509_FMT_PEM);
                if ( ret < 0 ) {
                ...
                for ( i = 0; i < cert_max; i++ ) {
                        ...
                        requiem_connection_permission_to_string(permission, str);
                        print_add(i, dh->d_name, uidbuf, gidbuf, analyzerid, requiem_string_get_string(str), buf);
...
static int print_detailed_help(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
        unsigned int i;
...
int main(int argc, char **argv)
{
        ...
        arg_command = argv[1];
        requiem_init(NULL, NULL);

        ret = requiem_option_new(NULL, &parentopt);
        requiem_option_add(parentopt, NULL, REQUIEM_OPTION_TYPE_CLI, 'h', "help",
                           NULL, REQUIEM_OPTION_ARGUMENT_NONE, print_detailed_help, NULL);
