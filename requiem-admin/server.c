...
static int anon_check_passwd(requiem_io_t *fd)
{
        ssize_t ret;
        const char *result;
        unsigned char *rbuf;
        ...
        ret = requiem_io_write_delimited(fd, result, strlen(result));
        if ( ret < 0 ) {
...
static int handle_client_connection(const char *srcinfo, requiem_client_profile_t *cp, requiem_io_t *fd,
                                    gnutls_x509_privkey key, gnutls_x509_crt cacrt, gnutls_x509_crt crt)
{
        gnutls_session session;

        session = new_tls_session(requiem_io_get_fd(fd));
        if ( ! session )
                return -1;
...
static int process_event(requiem_client_profile_t *cp, int server_sock, requiem_io_t *fd,
                         gnutls_x509_privkey key, gnutls_x509_crt cacrt, gnutls_x509_crt crt)
{
        char buf[512];
        void *inaddr;
...
# define ADDR_PORT(x) (x).sin6_port
        ...
        inaddr = requiem_sockaddr_get_inaddr(&addr.sa);
        if ( ! inaddr )
                return -1;
        ...
        requiem_io_set_sys_io(fd, csock);
                        ...
                        gnutls_datum *verifier, gnutls_datum *generator, gnutls_datum *prime)
{
        ...
        if ( strcmp(username, "requiem-adduser") != 0 )
                return -1;
...
int server_create(requiem_client_profile_t *cp, const char *addr, unsigned int port,
                  requiem_bool_t keepalive, const char *pass, gnutls_x509_privkey key, gnutls_x509_crt cacrt, gnutls_x509_crt crt)
{
        int sock;
        size_t size;
