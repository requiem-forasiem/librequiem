...
/* Return a pointer to a string containing the name of the symbol of
   ...
   code is not known.  */
const char *requiem_strsource_sym(requiem_error_t err)
{
  	requiem_error_source_t source = requiem_error_source(err);

  	if ( msgidxof(source) == msgidxof(REQUIEM_ERROR_SOURCE_DIM) )
    		return NULL;
