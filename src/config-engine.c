...
#define REQUIEM_ERROR_SOURCE_DEFAULT REQUIEM_ERROR_SOURCE_CONFIG_ENGINE
...
struct config {
        ...
        requiem_bool_t need_sync;  /* does the file need to be synced on disk ? */
...
static requiem_bool_t is_line_commented(const char *line)
{
        line += strspn(line, " \t\r");
 ...
 */
static requiem_bool_t is_section(const char *line)
{
        line += strspn(line, " \t\r\n");
...
static int op_delete_line(config_t *cfg, unsigned int start, unsigned int end)
{
        ...
        cfg->content = _requiem_realloc(cfg->content, cfg->elements * sizeof(char **));
        if ( ! cfg->content )
...
static int load_file_in_memory(config_t *cfg)
{
        ...
        if ( ! fd ) {
                ...
                return requiem_error_verbose(requiem_error_code_from_errno(errno), "could not open '%s' for reading: %s",
        ...
        do {
                ...
                if ( ptr ) {
                        ...
                        ret = requiem_string_cat(out, line);
                        if ( ret < 0 )
                                goto err;
                ...
                ret = requiem_string_get_string_released(out, &tmp);
                if ( ret < 0 )
                        goto err;
...
{
        size_t slen, elen;
        requiem_bool_t have_start_quote = FALSE;
        ...
        *out = strndup(in, elen);
        ...
        return (*out) ? 0 : requiem_error_from_errno(errno);
}
        ...
        while ( *ptr ) {
                if ( *ptr == '$' ) {
                        ...
                        if ( ! variable_get(buf + 1) )
                                ...
                                requiem_string_cat(out, variable_get(buf + 1));
                                continue;
                        }
                }

                requiem_string_ncat(out, ptr, 1);
                ptr++;
        }

        ret = requiem_string_get_string_released(out, out_p);
        requiem_string_destroy(out);
...
static int parse_section_buffer(const char *buf, char **entry, char **value, requiem_bool_t default_value)
{
        int ret;
        char *ptr;
...
static int sync_and_free_file_content(config_t *cfg)
{
        ...
        if ( ! fd )
                return requiem_error_verbose(requiem_error_code_from_errno(errno), "could not open '%s' for writing: %s",
                                             cfg->filename, strerror(errno));
        ...
        for ( i = 0; i < cfg->elements; i++ ) {
                ...
                if ( ret != len && ferror(fd) )
                        requiem_log(REQUIEM_LOG_ERR, "error writing content to '%s': %s", cfg->filename, strerror(errno));
