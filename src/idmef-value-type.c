...
#define REQUIEM_ERROR_SOURCE_DEFAULT REQUIEM_ERROR_SOURCE_IDMEF_VALUE_TYPE
...
#define GENERIC_ONE_BASE_RW_FUNC(scanfmt, printfmt, name, type)                          \
        ...
        {                                                                                \
                ...
                return (ret == 1) ? 0 : requiem_error_verbose(REQUIEM_ERROR_IDMEF_VALUE_TYPE_PARSE, \
                                                              "Reading " #name " value failed");    \
        }                                                                                \
                                                                                         \
        static int name ## _write(const idmef_value_type_t *src, requiem_string_t *out)  \
        {                                                                                \
                return requiem_string_sprintf(out, (printfmt), src->data.name ##_val);   \
        }
...
#define GENERIC_TWO_BASES_RW_FUNC(fmt_dec, fmt_hex, name, type)                         \
        ...
        static int name ## _write(const idmef_value_type_t *src, requiem_string_t *out) \
        {                                                                               \
                return requiem_string_sprintf(out, (fmt_dec), src->data.name ##_val);   \
        }
...
typedef struct {
        ...
        int (*write)(const idmef_value_type_t *src, requiem_string_t *out);
...
static int byte_read(idmef_value_type_t *dst, const char *buf, unsigned int min, unsigned int max)
{
        ...
        if ( buf == endptr || tmp < min || tmp > max )
                return requiem_error_verbose(REQUIEM_ERROR_IDMEF_VALUE_TYPE_PARSE,
...
static int int8_write(const idmef_value_type_t *src, requiem_string_t *out)
{
        return requiem_string_sprintf(out, "%d", (int) src->data.int8_val);
}

static int uint8_write(const idmef_value_type_t *src, requiem_string_t *out)
{
        return requiem_string_sprintf(out, "%u", (int) src->data.int8_val);
}

static int int8_read(idmef_value_type_t *dst, const char *buf)
{
        return byte_read(dst, buf, REQUIEM_INT8_MIN, REQUIEM_INT8_MAX);
}
...
GENERIC_TWO_BASES_RW_FUNC("%" REQUIEM_PRId64, "%" REQUIEM_PRIx64, int64, int64_t)
GENERIC_TWO_BASES_RW_FUNC("%" REQUIEM_PRIu64, "%" REQUIEM_PRIx64, uint64, uint64_t)
...
static int enum_read(idmef_value_type_t *dst, const char *buf)
{
        ...
        return (ret == 1) ? 0 : requiem_error_verbose(REQUIEM_ERROR_IDMEF_VALUE_TYPE_PARSE, "Reading enum value failed");
}



static int enum_write(const idmef_value_type_t *src, requiem_string_t *out)
{
        const char *str;
        ...
        return requiem_string_cat(out, str);
}
...
static int time_write(const idmef_value_type_t *src, requiem_string_t *out)
{
                          ...
                          size_t size, idmef_criterion_operator_t op)
{
        ...
        if ( t1->data.string_val )
                s1 = requiem_string_get_string(t1->data.string_val);

        if ( t2->data.string_val )
                s2 = requiem_string_get_string(t2->data.string_val);
...
{
        return requiem_string_new_dup(&dst->data.string_val, buf);
}
...
{
        return requiem_string_copy_dup(src->data.string_val, dst);
}


static int string_ref(const idmef_value_type_t *src)
{
        requiem_string_ref(src->data.string_val);
        return 0;
}
...
{
        return requiem_string_clone(src->data.string_val, &dst->data.string_val);
}


static void string_destroy(idmef_value_type_t *type)
{
        requiem_string_destroy(type->data.string_val);
}



static int string_write(const idmef_value_type_t *src, requiem_string_t *out)
{
        return requiem_string_sprintf(out, "%s",
                                      requiem_string_get_string(src->data.string_val));
}
...
static int data_write(const idmef_value_type_t *src, requiem_string_t *out)
{
...
static int is_type_valid(idmef_value_type_id_t type)
{
        if ( type < 0 || (size_t) type >= (sizeof(ops_tbl) / sizeof(*ops_tbl)) )
                return requiem_error_verbose(REQUIEM_ERROR_IDMEF_VALUE_TYPE_UNKNOWN, "Unknown IDMEF type id: '%d'", type);
