...
#define REQUIEM_ERROR_SOURCE_DEFAULT REQUIEM_ERROR_SOURCE_IDMEF_PATH
...
typedef struct {
        ...
        requiem_bool_t reversed;
        requiem_bool_t delete_list;
} value_list_t;
...
static requiem_bool_t flush_cache = FALSE;
static requiem_hash_t *cached_path = NULL;
...
static int initialize_path_cache_if_needed(void)
{
        ...
        return requiem_hash_new2(&cached_path, HASH_DEFAULT_SIZE, NULL, NULL, NULL, flush_cache_if_wanted);
}
...
static int build_name(idmef_path_t *path)
{
        ...
        for ( i = 0; i < path->depth; i++ ) {
                ...
                if ( ! name )
                        return requiem_error(REQUIEM_ERROR_IDMEF_PATH_INTEGRITY);
...
static int idmef_path_get_list_internal(idmef_value_t **value_list,
                                        ...
                                        requiem_list_t *list, idmef_class_id_t parent_class)
{
        int ret;
        requiem_list_t *tmp;
        idmef_value_t *value;
...
static int idmef_path_get_nth_internal(idmef_value_t **value, const idmef_path_t *path,
                                       unsigned int depth, requiem_list_t *list,
                                       idmef_class_id_t parent_class, int which)
{
        ...
        if ( which >= 0 ) {
                ...
                requiem_list_for_each_reversed(list, tmp) {
                        if ( cnt++ == which )
                                return idmef_path_get_internal(value, path, depth, tmp, parent_class);
                }
        }
...
static void delete_listed_child(void *parent, idmef_class_id_t class, const idmef_path_element_t *elem)
{
        ...
        requiem_list_for_each_safe(head, tmp, bkp) {
                 ...
                 *
                 * We manually call requiem_list_del_init() in order to
                 * disassociate the object from the message.
                 */
                requiem_list_del_init(tmp);
                idmef_class_destroy(idmef_class_get_child_class(class, elem->position), obj);
        }
}

static int _idmef_path_set(const idmef_path_t *path, idmef_message_t *message, idmef_value_t *value, requiem_bool_t *delete_list)
{
        size_t i;
        void *ptr;
        int ret, index;
        ...
        for ( i = 0; i < path->depth; i++ ) {
                ...
                if ( i + 1 < path->depth && index == INDEX_UNDEFINED && idmef_class_is_child_list(class, elem->position) )
                        return requiem_error_verbose(REQUIEM_ERROR_IDMEF_PATH_MISS_INDEX,
...
{
        requiem_bool_t delete_list = TRUE;

        if ( path->depth < 1 )
                return requiem_error_verbose(REQUIEM_ERROR_GENERIC, "Path with depth of 0 are not allowed");
...
static int idmef_path_create(idmef_path_t **path, const char *buffer)
{
        ...
        *path = requiem_hash_get(cached_path, buffer);
        gl_lock_unlock(cached_path_mutex);
...
static int idmef_path_parse_new(idmef_path_t *path, const char *buffer)
{
        ...
        if ( len >= sizeof(path->name) )
                return requiem_error(REQUIEM_ERROR_IDMEF_PATH_LENGTH);
        ...
        do {
                ...
                if ( index == INDEX_UNDEFINED )
                        ...
                        if ( ! idmef_class_is_child_list(class, child) )
                                return requiem_error_verbose(REQUIEM_ERROR_IDMEF_PATH_INDEX_FORBIDDEN,
                ...
                if ( ++depth == MAX_DEPTH )
                        return requiem_error(REQUIEM_ERROR_IDMEF_PATH_DEPTH);
...
int idmef_path_new_fast(idmef_path_t **path, const char *buffer)
{
        ...
        requiem_return_val_if_fail(buffer, requiem_error(REQUIEM_ERROR_ASSERTION));
        ...
        if ( requiem_hash_set(cached_path, (*path)->name, *path) < 0 ) {
...
int idmef_path_new_v(idmef_path_t **path, const char *format, va_list args)
{
        ...
        requiem_return_val_if_fail(format, requiem_error(REQUIEM_ERROR_ASSERTION));
...
{
        requiem_return_val_if_fail(path, requiem_error(REQUIEM_ERROR_ASSERTION));
...
static inline int invalidate(idmef_path_t *path)
{
        ...
        if ( path->refcount == 2 ) {
                gl_lock_lock(cached_path_mutex);
                ret = requiem_hash_elem_destroy(cached_path, path->name);
                gl_lock_unlock(cached_path_mutex);
...
int idmef_path_set_index(idmef_path_t *path, unsigned int depth, int index)
{
        ...
        requiem_return_val_if_fail(path, requiem_error(REQUIEM_ERROR_ASSERTION));
        requiem_return_val_if_fail(depth < path->depth, requiem_error(REQUIEM_ERROR_IDMEF_PATH_DEPTH));

        if ( index == INDEX_FORBIDDEN )
                return requiem_error(REQUIEM_ERROR_IDMEF_PATH_INDEX_RESERVED);

        if ( path->elem[depth].index == INDEX_FORBIDDEN )
                return requiem_error(REQUIEM_ERROR_IDMEF_PATH_INDEX_FORBIDDEN);
...
int idmef_path_get_index(const idmef_path_t *path, unsigned int depth)
{
        ...
        if ( path->elem[depth].index == INDEX_UNDEFINED )
                return requiem_error(REQUIEM_ERROR_IDMEF_PATH_INDEX_UNDEFINED);
...
int idmef_path_make_child(idmef_path_t *path, const char *child_name, int index)
{
        ...
        requiem_return_val_if_fail(path, requiem_error(REQUIEM_ERROR_ASSERTION));
        requiem_return_val_if_fail(child_name, requiem_error(REQUIEM_ERROR_ASSERTION));
...
int idmef_path_make_parent(idmef_path_t *path)
{
        ...
        if ( path->depth == 0 )
                return requiem_error(REQUIEM_ERROR_IDMEF_PATH_PARENT_ROOT);
...
int idmef_path_ncompare(const idmef_path_t *p1, const idmef_path_t *p2, unsigned int depth)
{
        ...
        requiem_return_val_if_fail(p1, requiem_error(REQUIEM_ERROR_ASSERTION));
        requiem_return_val_if_fail(p2, requiem_error(REQUIEM_ERROR_ASSERTION));
...
{
        requiem_return_val_if_fail(path && result, requiem_error(REQUIEM_ERROR_ASSERTION));
...
{
        requiem_return_val_if_fail(path, NULL);
 ...
 */
requiem_bool_t idmef_path_is_ambiguous(const idmef_path_t *path)
{
        size_t i;

        requiem_return_val_if_fail(path, FALSE);
...
int idmef_path_has_lists(const idmef_path_t *path)
{
        ...
        requiem_return_val_if_fail(path, 0);
...
requiem_bool_t idmef_path_is_list(const idmef_path_t *path, int depth)
{
        requiem_return_val_if_fail(path, FALSE);
...
const char *idmef_path_get_name(const idmef_path_t *path, int depth)
{
        ...
        requiem_return_val_if_fail(path, NULL);
        requiem_return_val_if_fail(depth < 0 || (size_t) depth < path->depth, NULL);
...
void _idmef_path_cache_lock(void)
{
        ...
        if ( cached_path )
                requiem_hash_iterate(cached_path, path_lock_cb);
}
...
void _idmef_path_cache_reinit(void)
{
        ...
        if ( cached_path )
                requiem_hash_iterate(cached_path, path_reinit_cb);
}
...
{
        if ( cached_path )
                requiem_hash_iterate(cached_path, path_unlock_cb);
...
void _idmef_path_cache_destroy(void)
{
        ...
        flush_cache = TRUE;
        requiem_hash_destroy(cached_path);
        flush_cache = FALSE;
}
