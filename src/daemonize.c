...
static int lockfile_get_exclusive(const char *lockfile)
{
        ...
        if ( ret < 0 ) {
                if ( errno == EACCES || errno == EAGAIN )
                        return requiem_error_verbose(REQUIEM_ERROR_DAEMONIZE_LOCK_HELD,
 ...
 */
int requiem_daemonize(const char *lockfile)
{
        pid_t pid;
        int fd = 0, ret, i;
...
#if (defined _WIN32 || defined __WIN32__) && !defined __CYGWIN__
        requiem_log(REQUIEM_LOG_ERR, "Daemonize call unsupported in this environment.\n");
        pid = getpid();
#else
        pid = fork();
        if ( pid < 0 )
        ...
        ret = chdir("/");
        if ( ret < 0 )
                requiem_log(REQUIEM_LOG_ERR, "could not change working directory to '/': %s.\n", strerror(errno));
