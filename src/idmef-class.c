...
#define REQUIEM_ERROR_SOURCE_DEFAULT REQUIEM_ERROR_SOURCE_IDMEF_TYPE
...
static inline int is_class_valid(idmef_class_id_t class)
{
        if ( class < 0 || (size_t) class >= sizeof(object_data) / sizeof(*object_data) )
                return requiem_error_verbose(REQUIEM_ERROR_IDMEF_CLASS_UNKNOWN, "Unknown IDMEF class '%d'", (int) class);
...
static inline int is_child_valid(idmef_class_id_t class, idmef_class_child_id_t child)
{
        ...
        if ( child < 0 || (size_t) child >= object_data[class].children_list_elem )
                return requiem_error_verbose(REQUIEM_ERROR_IDMEF_CLASS_UNKNOWN_CHILD, "Unknown IDMEF child '%d' for class '%s'",
...
idmef_class_child_id_t idmef_class_find_child(idmef_class_id_t class, const char *name)
{
        ...
        return requiem_error_verbose(REQUIEM_ERROR_IDMEF_CLASS_UNKNOWN_CHILD, "Unknown IDMEF child '%s'", name);
}




requiem_bool_t idmef_class_is_child_list(idmef_class_id_t class, idmef_class_child_id_t child)
{
        int ret;
...
idmef_class_id_t idmef_class_get_child_class(idmef_class_id_t class, idmef_class_child_id_t child)
{
        ...
        if ( c->type != IDMEF_VALUE_TYPE_CLASS && c->type != IDMEF_VALUE_TYPE_ENUM )
                return requiem_error(REQUIEM_ERROR_IDMEF_CLASS_CHILD_NOT_CLASS);
...
idmef_class_id_t idmef_class_find(const char *name)
{
        ...
        return requiem_error_verbose(REQUIEM_ERROR_IDMEF_CLASS_UNKNOWN_NAME, "Unknown IDMEF class '%s'", name);
}
