...
int idmef_time_set_from_string(idmef_time_t *time, const char *buf)
{
        ...
        char *ptr;
        struct tm tm;
        requiem_bool_t miss_gmt = TRUE;

        requiem_return_val_if_fail(time, requiem_error(REQUIEM_ERROR_ASSERTION));
        ...
        if ( ! ptr )
                return requiem_error_verbose(REQUIEM_ERROR_GENERIC, "error parsing date field, format should be: YY-MM-DD");

        if ( *ptr ) {
                ptr = parse_time_hmsu(&tm, &time->usec, ptr);
                if ( ! ptr )
                        return requiem_error_verbose(REQUIEM_ERROR_GENERIC, "error parsing time field, format should be: HH:MM:SS");

                if ( *ptr ) {
                        ret = parse_time_gmt(&tm, &time->gmt_offset, ptr);
                        if ( ret < 0 )
                                return requiem_error_verbose(REQUIEM_ERROR_GENERIC, "error parsing GMT offset field (Z)?(+|-)?HH:MM");
        ...
        if ( miss_gmt ) {
                long gmtoff;
                requiem_get_gmt_offset_from_tm(&tm, &gmtoff);
                time->gmt_offset = (int32_t) gmtoff;
        }

        time->sec = miss_gmt ? mktime(&tm) : requiem_timegm(&tm);
        return 0;
}
 ...
 * @out: Pointer to a #requiem_string_t output buffer.
 *
 ...
 */
int idmef_time_to_ntpstamp(const idmef_time_t *time, requiem_string_t *out)
{
        l_fp ts;
        struct timeval tv;
        ...
        ret = requiem_string_sprintf(out, "0x%08lx.0x%08lx", (unsigned long) ts.l_ui, (unsigned long) ts.l_uf);
 ...
 * Returns: number of bytes written on success, a negative value if an error occured.
 */
int idmef_time_to_string(const idmef_time_t *time, requiem_string_t *out)
{
        time_t t;
        struct tm utc;
        ...
        return requiem_string_sprintf(out, "%d-%.2d-%.2dT%.2d:%.2d:%.2d.%02u%+.2d:%.2d",
...
int idmef_time_set_from_timeval(idmef_time_t *time, const struct timeval *tv)
{
        ...
        ret = requiem_get_gmt_offset_from_time((const time_t *) &tv->tv_sec, &gmtoff);
        if ( ret < 0 )
                return ret;
...
{
        requiem_return_val_if_fail(time, NULL);
...
void idmef_time_set_from_time(idmef_time_t *time, const time_t *t)
{
        ...
        requiem_return_if_fail(time);
        requiem_return_if_fail(t);

        requiem_get_gmt_offset_from_time(t, &gmtoff);
...
{
        requiem_return_val_if_fail(time, 0);
