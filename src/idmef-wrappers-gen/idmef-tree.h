 ...
 *    to retrieve the list entry, thus it assumes that the requiem_list_t
...
#ifndef _LIBREQUIEM_IDMEF_TREE_H
#define _LIBREQUIEM_IDMEF_TREE_H
...
#include "requiem-message.h"

#define LISTED_OBJECT(name, type) requiem_list_t name

#define IS_LISTED requiem_list_t list
...
PRIMITIVE_TYPE_STRUCT(requiem_string_t)
...
struct {
        ...
        REQUIRED(requiem_string_t, *name);
        REQUIRED(requiem_string_t, *url);
        requiem_string_t *meaning;
...
struct {
        REFCOUNT;
        requiem_string_t *ident;
        REQUIRED(requiem_string_t, *text);
...
struct {
        ...
        requiem_string_t *tty;
        requiem_string_t *name;
...
struct {
        ...
        requiem_string_t *vlan_name;
        OPTIONAL_INT(int32_t, vlan_num);
        REQUIRED(requiem_string_t, *address);
        requiem_string_t *netmask;
...
struct {
        ...
        requiem_string_t *path;

        LISTED_OBJECT(arg_list, requiem_string_t);
        LISTED_OBJECT(env_list, requiem_string_t);
...
struct {
        REFCOUNT;
        REQUIRED(requiem_string_t, *url);
        requiem_string_t *cgi;
        requiem_string_t *http_method;
        LISTED_OBJECT(arg_list, requiem_string_t);
...
struct {
        REFCOUNT;
        requiem_string_t *oid;
        OPTIONAL_INT(uint32_t, message_processing_model);
        OPTIONAL_INT(uint32_t, security_model);
        requiem_string_t *security_name;
        OPTIONAL_INT(uint32_t, security_level);
        requiem_string_t *context_name;
        requiem_string_t *context_engine_id;
        requiem_string_t *command;

        /*
         * FIXME: deprecated
         */
        requiem_string_t *community;
...
struct {
        ...
        requiem_string_t *iana_protocol_name;
        
        requiem_string_t *name;
        OPTIONAL_INT(uint16_t, port);
        requiem_string_t *portlist;
        requiem_string_t *protocol;
...
struct {
        ...
        LISTED_OBJECT(permission_list, requiem_string_t);
...
ENUM() {
        ...
        IDMEF_CHECKSUM_ALGORITHM_MD4(MD4)   =  1,
        IDMEF_CHECKSUM_ALGORITHM_MD5(MD5) = 2,
        IDMEF_CHECKSUM_ALGORITHM_SHA1(SHA1) = 3,
        IDMEF_CHECKSUM_ALGORITHM_SHA2_256(SHA2-256) = 4,
        IDMEF_CHECKSUM_ALGORITHM_SHA2_384(SHA2-384) = 5,
        IDMEF_CHECKSUM_ALGORITHM_SHA2_512(SHA2-512) = 6,
        IDMEF_CHECKSUM_ALGORITHM_CRC_32(CRC-32) = 7,
        IDMEF_CHECKSUM_ALGORITHM_HAVAL(Haval) = 8,
        IDMEF_CHECKSUM_ALGORITHM_TIGER(Tiger) = 9,
        IDMEF_CHECKSUM_ALGORITHM_GOST(Gost) = 11
...
struct {
        IS_LISTED;
        REFCOUNT;
        REQUIRED(requiem_string_t, *value);
...
struct {
        ...
        REQUIRED(requiem_string_t, *name);
        REQUIRED(requiem_string_t, *path);
...
struct {
        IS_LISTED;
        REFCOUNT;
        requiem_string_t *analyzerid;
        
        requiem_string_t *name;
        requiem_string_t *manufacturer;
        requiem_string_t *model;
        requiem_string_t *version;
        requiem_string_t *class;
        requiem_string_t *ostype;
        requiem_string_t *osversion;
...
struct {
        ...
        REQUIRED(requiem_string_t, *alertident);
        requiem_string_t *analyzerid;
...
struct {
        ...
        REQUIRED(requiem_string_t, *program);
...
struct {        
        ...
        REQUIRED(requiem_string_t, *version);
        ...
        HIDE(requiem_msg_t *, pmsg);
        
} TYPE_ID(idmef_message_t, 49);

#endif /* _LIBREQUIEM_IDMEF_TREE_H */
