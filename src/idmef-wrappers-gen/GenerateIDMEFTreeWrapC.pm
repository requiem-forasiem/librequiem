...
");
    ...
    foreach my $field ( map { $_->{metatype} & &METATYPE_LIST ? $_ : () } @{ $struct->{field_list} } ) {
        ...
        $self->output("
        requiem_list_init(&(*ret)->$field->{name});
...
");
    ...
    foreach my $field ( @{ $struct->{field_list} } ) {
        ...
        if ( $field->{typename} ne "requiem_string_t" and !($field->{metatype} & (&METATYPE_PRIMITIVE)) ) {
                $prefix = "idmef_";
        }
...
");
                ...
                #$self->output("                int retval = requiem_string_new_ref_fast(&(*ret)->$field->{name}, \"1.0\", 3);");
        #} else {
    ...
    foreach my $field ( @{ $struct->{field_list} } ) {
        ...
        if ( $field->{metatype} & &METATYPE_LIST ) {
                ...
                if ( $field->{typename} eq "idmef_time_t" ||
                     ...
                     $field->{typename} eq "requiem_string_t" ) {
            ...
            $self->output("
                case $n: \{
                        ...
                        if ( n >= 0 ) {
                               requiem_list_for_each(&ptr->$field->{name}, tmp) {
                                       if ( i++ == n ) {
                                               void *b = requiem_list_entry(tmp, $field->{typename}, list);
                                               ${type}_destroy(b);
                                               return 0;
                                       }
                               }
                               ...
                               requiem_list_for_each_reversed(&ptr->$field->{name}, tmp) {
                                       if ( i++ == pos ) {
                                               void *b = requiem_list_entry(tmp, $field->{typename}, list);
                                               ${type}_destroy(b);
                                               return 0;
            ...
            $self->output("
                case $n: \{
                        ...
                        if ( n >= 0 ) {
                               requiem_list_for_each(&ptr->$field->{name}, tmp) {
                                       if ( i++ == n ) {
                                               *ret = requiem_list_entry(tmp, $field->{typename}, list);
                                               return 0;
                                       }
                               }
...
");
    ...
    foreach my $field ( @{ $struct->{field_list} } ) {
        ...
        \{
                ...
                requiem_list_for_each_safe(&src->$field->{name}, tmp, n) \{
                        entry = requiem_list_entry(tmp, $field->{typename}, list);
                        $clone_func(entry, &new);
                        requiem_list_add_tail(&dst->$field->{name}, &new->list);
                \}
        \}
");
    ...
    foreach my $field ( @{ $struct->{field_list} } ) {
        ...
        \{
                ...
                do \{
                        ...
                        requiem_list_for_each_continue(&obj1->$field->{name}, tmp1) \{
                                entry1 = requiem_list_entry(tmp1, $field->{typename}, list);
                                break;
                        \}

                        requiem_list_for_each_continue(&obj2->$field->{name}, tmp2) \{
                                entry2 = requiem_list_entry(tmp2, $field->{typename}, list);
                                break;
                        \}
...
");
    ...
    foreach my $field ( @{ $struct->{field_list} } ) {
        ...
        \{
                ...
                requiem_list_for_each_safe(&ptr->$field->{name}, tmp, n) \{
                        entry = requiem_list_entry(tmp, $field->{typename}, list);
...
\{
        requiem_return_val_if_fail($struct->{short_typename}, NULL);
...
\{
        ...
        requiem_return_if_fail(ptr->$field->{name});
...
");
                ...
                if ( $tree->{objs}->{$field->{typename}}->{is_listed} ) {
                     ...
                     $self->output("
        requiem_return_val_if_fail(ptr, requiem_error(REQUIEM_ERROR_ASSERTION));
        requiem_list_init(&ptr->$field->{name}.list);");
                 }
            }
        }
    }
...
\{
        requiem_list_t *tmp = ($field->{short_typename}_cur) ? &$field->{short_typename}_cur->list : NULL;

        requiem_return_val_if_fail($struct->{short_typename}, NULL);

        requiem_list_for_each_continue(&$struct->{short_typename}->$field->{name}, tmp)
                return requiem_list_entry(tmp, $field->{typename}, list);
...
\{
   ...
   $self->output("
        ...
        return requiem_error_verbose(REQUIEM_ERROR_IDMEF_UNKNOWN_ENUM_STRING, \"Unknown enumeration value '%s' for $enum->{short_typename}\", name);
\}
");
