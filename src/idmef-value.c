...
{                                                                    \
        requiem_return_val_if_fail(value, requiem_error(REQUIEM_ERROR_ASSERTION)); \
                                                                     \
        if ( value->own_data )                                       \
...
{                                                                \
        requiem_return_val_if_fail(val, (vtype) 0);              \
                                                                 \
...
#define VALUE_CAST_CHECK(v, itype_t, itype, iformat, ntype) do {      \
        ...
        if ( ntype == IDMEF_VALUE_TYPE_INT8 ) {                       \
                ...
                idmef_value_set_double(value, src);                   \
                                                                      \
        } else return requiem_error_verbose(REQUIEM_ERROR_GENERIC,    \
...
idmef_value_decl(STRING, string, requiem_string_t *)
...
{
        requiem_return_val_if_fail(value, requiem_error(REQUIEM_ERROR_ASSERTION));
        requiem_return_val_if_fail(object, requiem_error(REQUIEM_ERROR_ASSERTION));
...
{
        requiem_return_val_if_fail(list, requiem_error(REQUIEM_ERROR_ASSERTION));
        requiem_return_val_if_fail(item, requiem_error(REQUIEM_ERROR_ASSERTION));
...
requiem_bool_t idmef_value_is_list(const idmef_value_t *list)
{
        requiem_return_val_if_fail(list, FALSE);
        return (list->list) ? TRUE : FALSE;
}



requiem_bool_t idmef_value_list_is_empty(const idmef_value_t *list)
{
        requiem_return_val_if_fail(list, TRUE);
...
int idmef_value_new(idmef_value_t **value, idmef_value_type_id_t type, void *ptr)
{
        ...
        requiem_return_val_if_fail(ptr, requiem_error(REQUIEM_ERROR_ASSERTION));
...
static int idmef_value_set_own_data(idmef_value_t *value, requiem_bool_t own_data)
{
        int cnt;
...
{
        requiem_return_val_if_fail(value, NULL);
                        ...
                        int (*callback)(idmef_value_t *ptr, void *extra), void *extra)
{
        ...
        requiem_return_val_if_fail(value, requiem_error(REQUIEM_ERROR_ASSERTION));
        requiem_return_val_if_fail(callback, requiem_error(REQUIEM_ERROR_ASSERTION));
...
{
        requiem_return_val_if_fail(val, NULL);
...
{
        requiem_return_val_if_fail(val, requiem_error(REQUIEM_ERROR_ASSERTION));
...
int idmef_value_to_string(const idmef_value_t *val, requiem_string_t *out)
{
        requiem_return_val_if_fail(val, requiem_error(REQUIEM_ERROR_ASSERTION));
...
int idmef_value_print(const idmef_value_t *val, requiem_io_t *fd)
{
        int ret;
        ...
        return requiem_io_write(fd, requiem_string_get_string(out), requiem_string_get_len(out));
}
...
{
        requiem_return_val_if_fail(val, requiem_error(REQUIEM_ERROR_ASSERTION));
        requiem_return_val_if_fail(res, requiem_error(REQUIEM_ERROR_ASSERTION));
...
int idmef_value_match(idmef_value_t *val1, idmef_value_t *val2, idmef_criterion_operator_t op)
{
        ...
        requiem_return_val_if_fail(val1, requiem_error(REQUIEM_ERROR_ASSERTION));
        requiem_return_val_if_fail(val2, requiem_error(REQUIEM_ERROR_ASSERTION));
...
static int cast_to_data(idmef_value_t *input)
{
        ...
        if ( vtype == IDMEF_VALUE_TYPE_STRING ) {
                requiem_string_t *str = idmef_value_get_string(input);

                ret = idmef_data_new_char_string_dup_fast(&data, requiem_string_get_string(str), requiem_string_get_len(str));
                if ( ret < 0 )
                        return ret;
...
static int cast_to_time(idmef_value_t *value)
{
        ...
        if ( idmef_value_get_type(value) == IDMEF_VALUE_TYPE_STRING ) {
                ret = idmef_time_new_from_string(&time, requiem_string_get_string(idmef_value_get_string(value)));
                if ( ret < 0 )
                        return ret;
...
static int cast_from_string(idmef_value_t *value, idmef_value_type_id_t ntype)
{
        ...
        ret = idmef_value_type_read(&vt, requiem_string_get_string(str));
        if ( ret < 0 )
                return ret;
...
int _idmef_value_cast(idmef_value_t *value, idmef_value_type_id_t ntype, idmef_class_id_t id)
{
        ...
        requiem_log_debug(3, "converting '%s' to '%s'.\n",
        ...
        if ( ntype == IDMEF_VALUE_TYPE_DATA )
                ...
                requiem_string_t *str = idmef_value_get_string(value);
                return idmef_value_set_enum_from_string(value, id, requiem_string_get_string(str));
        }

        else if ( otype == IDMEF_VALUE_TYPE_INT8 )
                VALUE_CAST_CHECK(value, int8_t, int8, REQUIEM_PRId8, ntype);

        else if ( otype == IDMEF_VALUE_TYPE_UINT8 )
                VALUE_CAST_CHECK(value, uint8_t, uint8, REQUIEM_PRIu8, ntype);

        else if ( otype == IDMEF_VALUE_TYPE_INT16 )
                VALUE_CAST_CHECK(value, int16_t, int16, REQUIEM_PRId16, ntype);

        else if ( otype == IDMEF_VALUE_TYPE_UINT16 )
                VALUE_CAST_CHECK(value, uint16_t, uint16, REQUIEM_PRIu16, ntype);

        else if ( otype == IDMEF_VALUE_TYPE_INT32 )
                VALUE_CAST_CHECK(value, int32_t, int32, REQUIEM_PRId32, ntype);

        else if ( otype == IDMEF_VALUE_TYPE_UINT32 )
                VALUE_CAST_CHECK(value, uint32_t, uint32, REQUIEM_PRIu32, ntype);

        else if ( otype == IDMEF_VALUE_TYPE_INT64 )
                VALUE_CAST_CHECK(value, int64_t, int64, REQUIEM_PRId64, ntype);

        else if ( otype == IDMEF_VALUE_TYPE_UINT64 )
                VALUE_CAST_CHECK(value, uint64_t, uint64, REQUIEM_PRIu64, ntype);
