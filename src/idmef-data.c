...
{
        requiem_return_val_if_fail(data, NULL);
...
{
        requiem_return_val_if_fail(data, requiem_error(REQUIEM_ERROR_ASSERTION));
...
{
        requiem_return_val_if_fail(data, 0);
        return data->len;
}
 ...
 */
requiem_bool_t idmef_data_is_empty(const idmef_data_t *data)
{
        requiem_return_val_if_fail(data, TRUE);
        return (data->len == 0) ? TRUE : FALSE;
}



static int bytes_to_string(requiem_string_t *out, const unsigned char *src, size_t size)
{
        char c;
        int ret;
        ...
        while ( size ) {
                ret = requiem_string_ncat(out, &b64tbl[(src[0] >> 2) & 0x3f], 1);
                if ( ret < 0 )
                        return ret;

                c = b64tbl[((src[0] << 4) + ((--size) ? src[1] >> 4 : 0)) & 0x3f];

                ret = requiem_string_ncat(out, &c, 1);
                if ( ret < 0 )
                        return ret;
 ...
 * @data: Pointer to an #idmef_data_t object.
 * @out: Pointer to a #requiem_string_t to store the formated data into.
 *
 ...
 * Returns: 0 on success, a negative value if an error occured.
 */
int idmef_data_to_string(const idmef_data_t *data, requiem_string_t *out)
{
        int ret = 0;
        ...
        case IDMEF_DATA_TYPE_CHAR:
                ret = requiem_string_sprintf(out, "%c", data->data.char_data);
                break;
        ...
        case IDMEF_DATA_TYPE_BYTE:
                 ...
                 */
                ret = requiem_string_sprintf(out, "%u", (unsigned int) data->data.byte_data);
                break;

        case IDMEF_DATA_TYPE_UINT32:
                ret = requiem_string_sprintf(out, "%u", data->data.uint32_data);
                break;

        case IDMEF_DATA_TYPE_UINT64:
                ret = requiem_string_sprintf(out, "%" REQUIEM_PRIu64, data->data.uint64_data);
                break;

        case IDMEF_DATA_TYPE_FLOAT:
                ret = requiem_string_sprintf(out, "%f", data->data.float_data);
                break;

        case IDMEF_DATA_TYPE_CHAR_STRING:
                ret = requiem_string_sprintf(out, "%s", (const char *) data->data.ro_data);
                break;
