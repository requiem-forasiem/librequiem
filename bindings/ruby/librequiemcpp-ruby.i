...
static VALUE __requiem_log_func = Qnil;
...
static void _cb_ruby_log(int level, const char *str)
{
        ...
        rb_funcall(__requiem_log_func, cid, 2, SWIG_From_int(level), SWIG_FromCharPtr(str));
}


static int _cb_ruby_write(requiem_msgbuf_t *fd, requiem_msg_t *msg)
{
        FILE *f;
        ssize_t ret;
        OpenFile *fptr;
        VALUE *io = (VALUE *) requiem_msgbuf_get_data(fd);
...
static ssize_t _cb_ruby_read(requiem_io_t *fd, void *buf, size_t size)
{
        FILE *f;
        ssize_t ret;
        OpenFile *fptr;
        VALUE *io = (VALUE *) requiem_io_get_fdptr(fd);
...
%extend Requiem::IDMEF {
        ...
        Requiem::IDMEF &operator >> (void *nocast_p) {
                self->_genericWrite(_cb_ruby_write, nocast_p);
                return *self;
        }
...
VALUE IDMEFValueList_to_SWIG(const Requiem::IDMEFValue &value)
{
        VALUE ary;
        int ret, j = 0;
