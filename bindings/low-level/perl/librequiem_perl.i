...
void swig_perl_raise_error(int error)
{
	...
	snprintf(buf, sizeof(buf), "Requiem error - %s: %s", requiem_strsource(error), requiem_strerror(error));
	croak(buf);
}


SV *swig_perl_string(requiem_string_t *string)
{
	if ( string ) {
		return newSVpv(requiem_string_get_string(string), requiem_string_get_len(string));
	} else {
...
SV *swig_perl_data(idmef_data_t *data)
{
	...
	case IDMEF_DATA_TYPE_UINT64:
		return newSVpvf("%" REQUIEM_PRIu64, idmef_data_get_uint64(data));
...
%typemap(argout) (uint64_t *source_id, uint32_t *request_id, void **value) {
	...
	XPUSHs(sv_2mortal(newSVpvf("%" REQUIEM_PRIu64, *$1)));
	...
	if ( *$3 ) {
		...
		case REQUIEM_OPTION_REPLY_TYPE_LIST:
			XPUSHs(sv_2mortal(SWIG_NewPointerObj((void *) * $3, SWIG_TypeQuery("requiem_option_t *"), 0)));
			break;
		default:
...
%typemap(in) requiem_string_t * {
	...
	ret = requiem_string_new_dup_fast(&($1), str, len);
	if ( ret < 0 ) {
...
%typemap(argout) requiem_string_t *out {
	if ( result >= 0 ) {
		$result = newSVpv(requiem_string_get_string($1), requiem_string_get_len($1));
		argvi++;
	}
...
%typemap(in) requiem_msg_t **outmsg ($*1_type tmp) {
	tmp = NULL;
	$1 = ($1_ltype) &tmp;
};


%typemap(in) requiem_connection_t **outconn ($*1_type tmp) {
	tmp = NULL;
	$1 = ($1_ltype) &tmp;
};
