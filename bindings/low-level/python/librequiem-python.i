...
void swig_python_raise_exception(int error)
{
	...
	module = PyImport_ImportModule("requiem");
	exception_class = PyObject_GetAttrString(module, "RequiemError");
...
PyObject *swig_python_string(requiem_string_t *string)
{
	if ( string )
		return PyString_FromStringAndSize(requiem_string_get_string(string), requiem_string_get_len(string));
	else {
		Py_INCREF(Py_None);
		return Py_None;
	}
...
%typemap(argout) (uint64_t *source_id, uint32_t *request_id, void **value) {
	...
	if ( *$3 ) {
		switch ( ret ) {
		case REQUIEM_OPTION_REPLY_TYPE_LIST:
			value_obj = SWIG_NewPointerObj((void *) * $3, SWIG_TypeQuery("requiem_option_t *"), 0);
			break;
		default:
...
%typemap(in) requiem_string_t * {
       int ret;

       ret = requiem_string_new_dup_fast(&($1), PyString_AsString($input), PyString_Size($input));
       if ( ret < 0 ) {
...
%typemap(in, numinputs=0) requiem_string_t *out {
	int ret;
...
%typemap(argout) requiem_string_t *out {
	if ( result >= 0 )
		$result = PyString_FromStringAndSize(requiem_string_get_string($1), requiem_string_get_len($1));

	requiem_string_destroy($1);
};


%typemap(in, numinputs=0) requiem_msg_t **outmsg ($*1_type tmp) {
	tmp = NULL;
	$1 = ($1_ltype) &tmp;
};

%typemap(in, numinputs=0) requiem_connection_t **outconn ($*1_type tmp) {
	tmp = NULL;
	$1 = ($1_ltype) &tmp;
};
