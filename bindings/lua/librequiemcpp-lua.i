...
%ignore Set(Requiem::IDMEF &, int8_t);
%ignore Set(Requiem::IDMEF &, uint8_t);
%ignore Set(Requiem::IDMEF &, int16_t);
%ignore Set(Requiem::IDMEF &, uint16_t);
%ignore Set(Requiem::IDMEF &, int32_t);
%ignore Set(Requiem::IDMEF &, uint32_t);
%ignore Set(Requiem::IDMEF &, int64_t);
%ignore Set(Requiem::IDMEF &, uint64_t);
%ignore Set(Requiem::IDMEF &, float);
%ignore Set(Requiem::IDMEF &, std::string);
...
%ignore Requiem::IDMEFCriteria::operator const std::string() const;
%newobject Requiem::IDMEFCriteria::__str__;
%extend Requiem::IDMEFCriteria {
        char *__str__() { return my_strdup(self->ToString().c_str()); }
}

%ignore Requiem::IDMEFTime::operator const std::string() const;
%newobject Requiem::IDMEFTime::__str__;
%extend Requiem::IDMEFTime {
        char *__str__() { return my_strdup(self->ToString().c_str()); }
}

%ignore Requiem::IDMEF::operator const std::string() const;
%newobject Requiem::IDMEF::__str__;
...
static void _cb_lua_log(int level, const char *str)
{
        ...
        lua_rawgeti(__lua_state, LUA_REGISTRYINDEX, __requiem_log_func);
...
static int _cb_lua_write(requiem_msgbuf_t *fd, requiem_msg_t *msg)
{
        size_t ret;
        FILE *f = (FILE *) requiem_msgbuf_get_data(fd);
...
static ssize_t _cb_lua_read(requiem_io_t *fd, void *buf, size_t size)
{
        ssize_t ret;
        FILE *f = (FILE *) requiem_io_get_fdptr(fd);
...
%typemap(in, fragment="TransitionFunc") void (*log_cb)(int level, const char *log) {
        ...
        if ( __lua_state )
                luaL_unref(L, LUA_REGISTRYINDEX, __requiem_log_func);

        __requiem_log_func = luaL_ref(L, LUA_REGISTRYINDEX);
        $1 = _cb_lua_log;
        __lua_state = L;
};
...
int IDMEFValueList_to_SWIG(lua_State *L, const Requiem::IDMEFValue &value)
{
        bool is_list;
        int index = 0, ret;
...
int IDMEFValue_to_SWIG(lua_State* L, const IDMEFValue &result)
{
        ...
        if ( type == IDMEF_VALUE_TYPE_STRING ) {
                ...
                SWIG_FromCharPtrAndSize(requiem_string_get_string(str), requiem_string_get_len(str));
        }
                ...
                Requiem::IDMEFTime *time = new Requiem::IDMEFTime(idmef_time_ref(idmef_value_get_time(value)));
                SWIG_NewPointerObj(L, time, SWIGTYPE_p_Requiem__IDMEFTime, 1);
        }
...
%init {
        ...
        while ( lua_next(L, -2) != 0 ) {
                ...
                if ( idx >= ((sizeof(argv) / sizeof(char *)) - 1) )
                        throw RequiemError("Argument index too large");
